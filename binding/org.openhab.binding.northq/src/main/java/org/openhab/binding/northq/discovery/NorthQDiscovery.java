...
import org.openhab.binding.northq.handler.BindingHandlerInterface;
...
public class NorthQDiscovery extends AbstractDiscoveryService implements BindingHandlerInterface {
    ...
    private void addNorthQThing(NorthQThing thing) {
        ...
        if (roomMap.containsKey(thing.getRoom())) {
            roomId = String.format("%d%s%s", thing.getRoom(), NorthQBindingConstants.ROOM_ID_SEPERATOR,
        ...
        properties.put(NorthQBindingConstants.UNIQUE_ID, thing.getUniqueId());
        properties.put(NorthQBindingConstants.ROOM_ID, roomId);
